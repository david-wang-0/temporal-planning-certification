...
do {
  let network = undefined problem domain;
  (renaming, cert) \<leftarrow> (case certifier network of
...
definition check_and_cert where
"check_and_cert problem domain mode num_split dc certifier show_cert \<equiv> 
case make_certified_net problem domain certifier of 
  Result (network, renaming, cert) \<Rightarrow> do {
    res \<leftarrow> convert_check mode num_split dc network renaming cert show_cert;
    let _ = (case res of 
      Result r \<Rightarrow> undefined
    | Error es \<Rightarrow> undefined);
    return ()
  }
