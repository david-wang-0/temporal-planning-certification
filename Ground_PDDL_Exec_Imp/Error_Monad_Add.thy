(* VENDORED from Formal-PDDL-Semantics/Continuous_Planning/Error_Monad_Add.thy (verbatim).
   Copied here so the Analysis-free wf-checker slice the net builder uses does not pull the
   Analysis-tainted Continuous_Planning session.  Upstream home: FPS; to be moved into an
   Analysis-free FPS session later. *)
