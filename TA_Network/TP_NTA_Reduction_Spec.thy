...
proof (rule equalityI; rule subsetI)
  ...
  proof cases
    ...
    proof cases
      ...
      have s: "set (pre (at_start act)) \<union> set (dels (at_start act)) \<union> set (adds (at_start act)) \<subseteq> set props" using fluent_domain unfolding fluent_domain_def comp_apply act_ref_fluents_def using act(1) by simp
    ...
    next
      ...
      have s: "set (over_all act) \<subseteq> set props" using fluent_domain unfolding fluent_domain_def comp_apply using act(1) unfolding act_ref_fluents_def by simp
      have x': "x \<in> fst ` set (map (\<lambda>p. (PropLock p, 0, int (length actions))) props @ map (\<lambda>p. (PropVar p, 0, 1)) props)" using x s 
        unfolding set_map set_append by blast
    ...
    next
      ...
      have s: "set (pre (at_end act)) \<union> set (dels (at_end act)) \<union> set (adds (at_end act)) \<subseteq> set props" using fluent_domain unfolding fluent_domain_def comp_apply act_ref_fluents_def using act(1) by simp
      have x': "x \<in> fst ` set (map (\<lambda>p. (PropLock p, 0, int (length actions))) props @ map (\<lambda>p. (PropVar p, 0, 1)) props)" using x s 
        unfolding set_map set_append by blast
      have x'': "x  \<in> fold (\<union>) (map action_vars_spec actions) {} \<union> set (map PropVar init) \<union> set (map PropVar goal)" unfolding action_vars_spec_def using act(1) unfolding fold_union' Let_def using x 
        apply -
    ...
    subgoal for tr
      ...
      subgoal for trs
        ...
        subgoal 
          ...
          apply (elim imageE)
          subgoal for _ auto
            unfolding main_auto_init_edge_spec_def main_auto_goal_edge_spec_def Let_def start_edge_spec_def end_edge_spec_def edge_2_spec_def edge_3_spec_def
            by auto
          done
        done
      done
...
proof
  ...
  have trans_alt: "\<Union> (trans ` automaton_of ` (set actual_autos)) = all_ta_trans"
    ...
    subgoal for x
      unfolding all_ta_trans_def fold_union' ta_trans_def set_map
      unfolding automaton_of_def Simple_Network_Language.trans_def
      apply (subst image_image)
      apply (erule UnionE)
        unfolding comp_apply
        apply (erule imageE)
        ...
        subgoal for _ _ A
          apply (cases A)
          subgoal for _ _ trans 
            unfolding prod.case
            apply (rule UnionI)
             apply (rule imageI[where x = A])
             apply assumption
            by simp
          done
        done
      subgoal for x
        unfolding all_ta_trans_def fold_union' ta_trans_def set_map comp_apply image_image
        apply (erule UnionE)
        apply (erule imageE)
        subgoal for trans A
          apply (rule UnionI)
  ...
  show "(\<Union>g\<in>set (map (snd \<circ> snd \<circ> snd) actual_autos). fst ` set g) \<subseteq> Prod_TA_Defs.loc_set (set broadcast, map automaton_of actual_autos, map_of nta_vars)" find_theorems name: "TP*loc*correc" using actual_locs_correct unfolding actual_locs_def fold_union' ta_locs_def trans_to_locs_def Let_def ta_trans_def set_map
    apply (rule subsetI)
    subgoal for x
