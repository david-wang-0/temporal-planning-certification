...
(* instant actions *)
definition "instant_actions_before n \<equiv> set (filter (planning_sem.is_instant_action (time_index \<pi> i)) (actions_before n))"
...
definition "starting_actions_after n \<equiv> planning_sem.starting_actions_at (time_index \<pi> i) - starting_actions_before n"
...
definition "ending_actions_after n \<equiv> planning_sem.ending_actions_at (time_index \<pi> i) - ending_actions_before n"

(* all *)


lemma apply_snaps_is_app_effs[simp]: "apply_snaps = planning_sem.app_effs" 
  unfolding apply_snaps_def planning_sem.app_effs_def apply_effects_def by blast

lemma happ_combine': 
  assumes "S \<subseteq> happ_at planning_sem.happ_seq (time_index \<pi> i)"
      and "T \<subseteq> happ_at planning_sem.happ_seq (time_index \<pi> i)"
  shows "((apply_snaps T) o (apply_snaps S)) s = apply_snaps (S \<union> T) s"
  using planning_sem.happ_combine[of S T "time_index \<pi> i" s]
  using assms by auto
...
lemma starting_actions_after_0_is_starting_actions:
  "starting_actions_after 0 = planning_sem.starting_actions_at (time_index \<pi> i)"
...
lemma card_ending_actions_and_starting_before_less:
  assumes "is_starting_index (time_index \<pi> i) n"
      and "n < length actions"
    shows "card (planning_sem.ending_actions_at (time_index \<pi> i)) + card (starting_actions_before n) < length actions"
proof -
  have "planning_sem.ending_actions_at (time_index \<pi> i) \<union> planning_sem.starting_actions_at (time_index \<pi> i) \<subseteq> set actions" using planning_sem.all_actions_at[of "time_index \<pi> i"] by auto
  moreover
  have "disjnt (planning_sem.ending_actions_at (time_index \<pi> i)) (planning_sem.starting_actions_at (time_index \<pi> i))" 
    unfolding planning_sem.actions_at_defs  disjnt_def
    using planning_sem.action_happening_disj by blast
  ultimately
  have 1: "card (planning_sem.ending_actions_at (time_index \<pi> i)) + card (planning_sem.starting_actions_at (time_index \<pi> i)) \<le> length actions" 
  ...
  hence "card (starting_actions_before (Suc n)) \<le> card (planning_sem.starting_actions_at (time_index \<pi> i))" 
    using planning_sem.finite_starting_actions_at card_mono by auto
  hence "card (starting_actions_before n \<union> {actions ! n}) \<le> card (planning_sem.starting_actions_at (time_index \<pi> i))" 
    using assms starting_actions_before_Suc by simp
  hence "card (starting_actions_before n) < card (planning_sem.starting_actions_at (time_index \<pi> i))"
...
proof -
  have "actions ! n \<in> planning_sem.starting_actions_at (time_index \<pi> i)"
...
proof (cases "is_starting_index (time_index \<pi> i ) n")
  case True
...
lemma ending_actions_before_all_is_ending_actions: 
  "ending_actions_before (length actions) = planning_sem.ending_actions_at (time_index \<pi> i)"
...
lemma ending_snaps_before_all_is_ending_snaps: "ending_snaps_before (length actions) = planning_sem.ending_snaps_at (time_index \<pi> i)"
...
lemma apply_ending_snaps_before_all_is_apply_ending_snaps: "apply_ending_snaps_before (length actions) s = planning_sem.app_effs (planning_sem.ending_snaps_at (time_index \<pi> i)) s"
  unfolding planning_sem.app_effs_def apply_ending_snaps_before_def Let_def ending_snaps_before_all_is_ending_snaps apply_effects_def apply_snaps_def by blast
...
lemma ending_actions_after_0_is_ending_actions:
  "ending_actions_after 0 = planning_sem.ending_actions_at (time_index \<pi> i)"
...
lemma ending_actions_before_inv:
  assumes "n \<le> m"
      and "\<forall>j a. n \<le> j \<and> j < m \<longrightarrow> \<not>(is_ending_index (time_index \<pi> i) j)" 
...
lemma ending_actions_before_Suc:
  assumes "is_ending_index (time_index \<pi> i) n"
      and "n < length actions"
...
proof -
  have "ending_actions_before (Suc n) \<subseteq> planning_sem.ending_actions_at (time_index \<pi> i)" 
...
proof -
  have "actions ! n \<in> planning_sem.ending_actions_at (time_index \<pi> i)"
...
proof (cases "is_ending_index (time_index \<pi> i ) n")
  case True
...
lemma card_ending_actions_less:
  assumes "is_instant_index (time_index \<pi> i) n \<or> is_starting_index (time_index \<pi> i) n \<or> is_not_happening_index (time_index \<pi> i) n"
      and "n < length actions"
    shows "card (planning_sem.ending_actions_at (time_index \<pi> i)) < length actions"
proof -
  from assms
  have "actions ! n \<in> planning_sem.instant_actions_at (time_index \<pi> i) 
    \<or> actions ! n \<in> planning_sem.starting_actions_at (time_index \<pi> i) 
    \<or> actions ! n \<in> planning_sem.not_happening_actions_at (time_index \<pi> i)" unfolding index_case_defs planning_sem.actions_at_defs by simp
  with planning_sem.ending_actions_at_less
  have "planning_sem.ending_actions_at (time_index \<pi> i) \<subset> set actions" by auto
  thus ?thesis 
    apply (subst card_action_set[symmetric])
...
lemma card_starting_actions_and_ending_after_le:
  assumes "n \<le> length actions"
    shows "card (planning_sem.starting_actions_at (time_index \<pi> i)) + card (ending_actions_after n) \<le> length actions"
proof -
  have 1:"ending_actions_after n \<subseteq> planning_sem.ending_actions_at (time_index \<pi> i)" 
    using ending_actions_after_0_is_ending_actions ending_actions_after_inv_mono assms by blast

  
  have "disjnt (planning_sem.starting_actions_at (time_index \<pi> i)) (planning_sem.ending_actions_at (time_index \<pi> i))"
    unfolding disjnt_def planning_sem.actions_at_defs using planning_sem.action_happening_disj by blast
  hence 2: "disjnt (planning_sem.starting_actions_at (time_index \<pi> i)) (ending_actions_after n)" using 1  disjnt_subset2 by blast
  
  have 3: "planning_sem.starting_actions_at (time_index \<pi> i) \<union> ending_actions_after n \<subseteq> set actions"
...
proof (rule planning_sem.pre_sat_by_arbitrary_intermediate_state[simplified comp_apply, OF i t h])
  show "instant_part_updated_plan_state_seq n = planning_sem.app_effs (instant_snaps_before n) (planning_sem.plan_state_seq i)"
    unfolding instant_part_updated_plan_state_seq_def apply_instant_snaps_before_def apply_snaps_def planning_sem.app_effs_def apply_effects_def
    by blast
  show "instant_snaps_before n \<subseteq> happ_at planning_sem.happ_seq t"
  ...
  show "h \<notin> instant_snaps_before n"
    ...
    subgoal for b
      using at_start_inj[THEN inj_on_contraD, THEN notE, of "actions ! n" b]
      using actions_before_not_include n_in_act actions_before_in_actions n by auto
    using snaps_disj actions_before_in_actions n_in_act n apply blast
    using snaps_disj actions_before_in_actions n_in_act n apply blast
    subgoal for b
      using at_end_inj[THEN inj_on_contraD, THEN notE, of "actions ! n" b]
      using actions_before_not_include n_in_act actions_before_in_actions n by auto
    done
qed
...
proof -
  ...
  have 6: "instant_snaps_before n \<subseteq> happ_at planning_sem.happ_seq (time_index \<pi> i)" 
    unfolding instant_snaps_before_def instant_starts_before_def instant_ends_before_def 
      instant_actions_before_def planning_sem.action_happening_case_defs by auto
  have 7: "{at_start (actions ! n)} \<subseteq> happ_at planning_sem.happ_seq (time_index \<pi> i)" 
...
lemma pre_sat_by_starting_part_updated_state_seq:
  assumes t[simp]: "t = time_index \<pi> i"
      and h: "(t, h) \<in> planning_sem.happ_seq"
...
proof (rule planning_sem.pre_sat_by_arbitrary_intermediate_state[simplified comp_apply, OF i t h])
  have "starting_snaps_before n \<subseteq> happ_at planning_sem.happ_seq t" 
  ...
  moreover
  have "planning_sem.instant_snaps_at (time_index \<pi> i) \<subseteq> happ_at planning_sem.happ_seq t" using planning_sem.instant_snaps_happening t by blast
  ultimately
  show "planning_sem.instant_snaps_at (time_index \<pi> i) \<union> starting_snaps_before n \<subseteq> happ_at planning_sem.happ_seq t" by blast

  thus "starting_part_updated_state_seq n = planning_sem.app_effs (planning_sem.instant_snaps_at (time_index \<pi> i) \<union> starting_snaps_before n) (planning_sem.plan_state_seq i)"
  ...
  have n_in_act: "actions ! n \<in> set actions" using n by auto
  show "h \<notin> planning_sem.instant_snaps_at (time_index \<pi> i) \<union> starting_snaps_before n"
  proof (rule notI, elim UnE)
    assume "h \<in> planning_sem.instant_snaps_at (time_index \<pi> i)"
    thus False
      ...
      subgoal for b
        ...
        apply (erule conjE) 
        using h_start
        using at_start_inj[THEN inj_onD, of "actions ! n", OF _ n_in_act] by auto
      unfolding h_start 
      using snaps_disj n_in_act by blast
  next
    ...
    thus False 
      ...
      using n nth_actions_unique at_start_inj[THEN inj_on_contraD]
      by force
  qed
qed
...
lemma starting_part_updated_prop_state_inv:
  assumes "n \<le> m"
      and "\<forall>j a. n \<le> j \<and> j < m \<and> a = actions ! j \<longrightarrow> \<not>(planning_sem.is_starting_action (time_index \<pi> i) a)" 
    shows "starting_part_updated_prop_state n p = starting_part_updated_prop_state m p"
proof -
  have 1: "[0..<m] = [0..<n] @ [n..<m]" using assms upt_append by auto
  have 2: "filter (planning_sem.is_starting_action (time_index \<pi> i)) (map ((!) actions) [n..<m]) = []"
    apply (subst filter_empty_conv)
    using assms(2) by auto
  have "filter (planning_sem.is_starting_action (time_index \<pi> i)) (map ((!) actions) [0..<n]) =
      filter (planning_sem.is_starting_action (time_index \<pi> i)) (map ((!) actions) [0..<m])"
    apply (subst 1)
    using 2 by auto
  thus ?thesis 
...
lemma prop_state_after_instant_start_happ_is_prop_state_after_instant_happ_if_no_start:
  assumes "planning_sem.starting_actions_at (time_index \<pi> i) = {}"
  ...
  unfolding planning_sem.inst_start_upd_state_def planning_sem.inst_upd_state_def planning_sem.app_effs_def
  unfolding planning_sem.starting_snaps_at_def assms
  unfolding apply_effects_def 
  by simp
...
proof -
  ...
  proof -
    ...
    show ?thesis by auto
  qed
  hence 1: "at_end ` ending_actions_before n \<union> at_end ` {actions ! n} \<subseteq> happ_at planning_sem.happ_seq t"
...
proof (rule planning_sem.pre_sat_by_arbitrary_intermediate_state[simplified comp_apply, OF i t h])
  have "ending_snaps_before n \<subseteq> happ_at planning_sem.happ_seq t" 
  ...
  moreover
  have "planning_sem.instant_snaps_at t \<subseteq> happ_at planning_sem.happ_seq t" using planning_sem.instant_snaps_happening t by blast
  moreover
  have "planning_sem.starting_snaps_at t \<subseteq> happ_at planning_sem.happ_seq t" using planning_sem.starting_snaps_happening t by blast
  ultimately
  show "planning_sem.instant_snaps_at t \<union> planning_sem.starting_snaps_at t \<union> ending_snaps_before n \<subseteq> happ_at planning_sem.happ_seq t" by blast

  thus "ending_part_updated_state_seq n = planning_sem.app_effs (planning_sem.instant_snaps_at t \<union> planning_sem.starting_snaps_at t \<union> ending_snaps_before n) (planning_sem.plan_state_seq i)"
  ...
  proof (rule notI, elim UnE)
    ...
    thus False
      ...
      apply (elim UnE imageE CollectE conjE)
      unfolding h_end
      subgoal using snaps_disj n_in_act by blast
      subgoal for b
        apply (insert n_ending) 
        unfolding index_case_defs 
        apply (drule planning_sem.action_happening_disj)
        apply (erule conjE) 
        using at_end_inj[THEN inj_onD, of "actions ! n", OF _ n_in_act] by auto
      done
  next
    ...
    thus False 
      ...
      unfolding h_end using snaps_disj n_in_act by blast
  next
    ...
    thus False 
      ...
      using n nth_actions_unique at_end_inj[THEN inj_on_contraD]
      by force
  qed
qed
...
lemma ending_part_updated_prop_state_inv:
  assumes "n \<le> m"
      and "\<forall>j a. n \<le> j \<and> j < m \<and> a = actions ! j \<longrightarrow> \<not>(planning_sem.is_ending_action (time_index \<pi> i) a)" 
    shows "ending_part_updated_prop_state n p = ending_part_updated_prop_state m p"
proof -
  have 1: "[0..<m] = [0..<n] @ [n..<m]" using assms upt_append by auto
  have 2: "filter (planning_sem.is_ending_action (time_index \<pi> i)) (map ((!) actions) [n..<m]) = []"
    apply (subst filter_empty_conv)
    using assms(2) by auto
  have "filter (planning_sem.is_ending_action (time_index \<pi> i)) (map ((!) actions) [0..<n]) =
      filter (planning_sem.is_ending_action (time_index \<pi> i)) (map ((!) actions) [0..<m])"
    apply (subst 1)
    using 2 by auto
  thus ?thesis 
...
lemma prop_state_after_instant_start_happ_is_prop_state_after_happ_if_no_end:
  assumes "planning_sem.ending_actions_at (time_index \<pi> i) = {}"
...
lemma active_before_is_active_during_if_no_start:
  assumes "planning_sem.starting_actions_at (time_index \<pi> i) = {}"
  shows "planning_sem.active_before (time_index \<pi> i) = planning_sem.active_during (time_index \<pi> i)"
...
definition "updated_active_during n \<equiv>
  planning_sem.active_during (time_index \<pi> i) - card (ending_actions_before n)
"

lemma updated_active_during_0_is_active_during: "updated_active_during 0 = planning_sem.active_during (time_index \<pi> i)"
...
lemma updated_active_during_all_is_active_during_minus_ended: 
   "updated_active_during (length actions) = planning_sem.active_during_minus_ended (time_index \<pi> i)"
...
proof -
  have 1: "planning_sem.active_during (time_index \<pi> i) = planning_sem.active_during_minus_ended (time_index \<pi> i) + card (planning_sem.ending_actions_at (time_index \<pi> i))" 
    using planning_sem.active_during_conv_active_during_minus_ended by auto

  have 2: "card (ending_actions_before n) < card (planning_sem.ending_actions_at (time_index \<pi> i))" 
  proof -
    ...
    hence "(ending_actions_before n) \<union> {actions ! n} \<subseteq> planning_sem.ending_actions_at (time_index \<pi> i)" using ending_actions_before_Suc ending_actions_before_all_is_ending_actions assms by blast
    hence "(ending_actions_before n) \<subset> planning_sem.ending_actions_at (time_index \<pi> i)" 
    proof -
      have "disjnt (ending_actions_before n) {actions ! n}"
        unfolding disjnt_def ending_actions_before_def actions_before_def
        using nth_actions_unique assms(2) by auto
      thus "ending_actions_before n \<union> {actions ! n} \<subseteq> planning_sem.ending_actions_at (time_index \<pi> i) 
        \<Longrightarrow> ending_actions_before n \<subset> planning_sem.ending_actions_at (time_index \<pi> i)" by auto
    qed
...
lemma active_during_minus_ended_is_active_during_if_no_end:
  assumes "planning_sem.ending_actions_at (time_index \<pi> i) = {}"
  shows "planning_sem.active_during_minus_ended (time_index \<pi> i) = planning_sem.active_during (time_index \<pi> i)"
...
definition "updated_locked_during n p = planning_sem.locked_during (time_index \<pi> i) p + 
  card {a. a \<in> starting_actions_before n \<and> p \<in> (set o over_all) a}"

lemma updated_locked_during_all_is_locked_after:
  "updated_locked_during (length actions) p = planning_sem.locked_after (time_index \<pi> i) p"
proof -
  have "sum_list (map (\<lambda>a. if planning_sem.is_starting_action (time_index \<pi> i) a then (1::nat) else 0) (filter (\<lambda>a. p \<in> (set \<circ> over_all) a) planning_sem.action_list))
  = (\<Sum>a\<leftarrow>filter (\<lambda>a. planning_sem.is_starting_action (time_index \<pi> i) a) (filter (\<lambda>a. p \<in> (set \<circ> over_all) a) planning_sem.action_list). 1)
  + (\<Sum>a\<leftarrow>filter (\<lambda>a. \<not>planning_sem.is_starting_action (time_index \<pi> i) a) (filter (\<lambda>a. p \<in> (set \<circ> over_all) a) planning_sem.action_list). 0)" 
    using  sum_list_map_if'[where P = "(planning_sem.is_starting_action (time_index \<pi> i))"]
    by blast
  also have "... = (\<Sum>a\<leftarrow>filter (\<lambda>a. planning_sem.is_starting_action (time_index \<pi> i) a \<and> p \<in> (set \<circ> over_all) a) planning_sem.action_list. 1)" apply (subst sum_list_0) apply (subst filter_filter) apply (subst conj_commute) by simp
  also have "... = card {x \<in> set actions. planning_sem.is_starting_action (time_index \<pi> i) x \<and> p \<in> (set \<circ> over_all) x}" 
  ...
  finally have 1: "(\<Sum>a\<leftarrow>filter (\<lambda>a. p \<in> (set \<circ> over_all) a) planning_sem.action_list. if planning_sem.is_starting_action (time_index \<pi> i) a then 1 else 0) = card {a \<in> starting_actions_before (length actions). p \<in> (set \<circ> over_all) a}" by simp
...
lemma updated_locked_during_0_is_locked_during:
  "updated_locked_during 0 p = planning_sem.locked_during (time_index \<pi> i) p"
...
lemma updated_locked_during_Suc:
  assumes starting: "is_starting_index (time_index \<pi> i) n"
    and n: "n < length actions"
...
definition "leave_start_edge_effect n \<equiv> edge_effect (Suc n) (leave_start_edge_spec (actions ! n))"
...
lemma start_edge_effect_alt: "start_edge_effect n (L, v, c) = 
  (L[Suc n := StartInstant (actions ! n)],
     v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1, Effecting \<mapsto> plus_int (the (v Effecting)) 1,
...
lemma leave_start_edge_effect_alt: "leave_start_edge_effect n (L, v, c) = (L[Suc n := Running (actions ! n)], v, c)"
  unfolding leave_start_edge_effect_def leave_start_edge_spec_def
  unfolding  edge_effect_def Let_def prod.case unzip_def set_prop_ab_def
    map_map comp_def fst_map_prod snd_map_prod id_def list.map map_append fst_conv snd_conv
  by simp

lemma edge_2_effect_alt: "edge_2_effect n (L, v, c) = 
  (L[Suc n := PostStart (actions ! n)],
    v(Effecting \<mapsto> plus_int (the (v Effecting)) (-1),
        map PropLock (over_all (actions ! n)) [\<mapsto>] map (\<lambda>x. (the (v x)) + 1) (map PropLock (over_all (actions ! n)))), 
    c)"
...
lemma edge_3_effect_alt: "edge_3_effect n (L, v, c) = 
  (L[Suc n := EndInstant (actions ! n)],
    v(Effecting \<mapsto> plus_int (the (v Effecting)) 1,
      map PropLock (over_all (actions ! n)) [\<mapsto>] map (\<lambda>x. plus_int (the (v x)) (- 1)) (map PropLock (over_all (actions ! n)))),
...
definition "apply_leave_start_edge_effects ns s = seq_apply (map leave_start_edge_effect ns) s"
...
in [s] 
    ...
    |> ext_seq (apply_leave_start_edge_effects start_indices)
    |> tl
"
...
lemma map_of_nta_vars_Effecting:
  "map_of nta_vars Effecting = Some (0, int (length actions))" using nta_vars_exact by simp
...
lemma map_of_nta_vars_action_inv_props:
  assumes "a \<in> set actions"
    "v \<in> set (map PropVar (over_all a))"
  shows "map_of nta_vars v = Some (0, 1)"
proof -
  from assms local.planning_sem.finite_props_domain
  obtain p where
    p: "p \<in> set (over_all a)"
    "p \<in> set props"
    "v = PropVar p" unfolding fluent_domain_def act_ref_fluents_def by auto
  hence 1: "p \<in> set (filter (\<lambda>x. PropVar x \<in> \<Union> (set (map action_vars_spec actions))) props)" 
    unfolding action_vars_spec_def Let_def inv_vars_spec_def set_map using assms
    by auto

  have 2: "map_of (map (\<lambda>p. (PropVar p, y)) (filter (\<lambda>x. PropVar x \<in> S) props)) (PropVar p) 
    = (map_of (map (\<lambda>p. (p, y)) (filter (\<lambda>x. PropVar x \<in> S) props)) p)" for S y 
    apply (subst (2) map_of_map_inj_fst[symmetric, where f = PropVar])
    unfolding inj_def apply simp
    apply (subst map_map)
    apply (subst comp_def)
    unfolding prod.case
  ...
  show ?thesis
    ...
    apply fastforce
     apply (subst 2)
     apply (rule map_of_is_SomeI)
    using distinct_props unfolding map_map comp_apply 
    using 1 by auto
qed
...
proof -
  ...
  show ?thesis 
    ...
    apply (subst map_add_find_left)
     apply (rule map_of_NoneI)
    using 1 p unfolding set_map by auto
qed
...
proof -
  ...
  show ?thesis 
    ...
    apply (subst map_add_find_right)
    using 1 p(3) unfolding set_map by fast+
qed
...
lemma starting_action_sat_mutex_start:
  ...
  shows "c \<turnstile> g"
proof -
  have 1: "(t, at_start a) \<in> planning_sem.happ_seq" using starting planning_sem.is_starting_action_def by simp+
  have 2: "\<forall>b\<in>set actions. (t, at_start b) \<notin> planning_sem.happ_seq \<or> at_start a = at_start b \<longrightarrow> act_clock_pre_happ_spec c (ActStart b) t"
  apply (intro ballI impI)
    subgoal for b
      apply (erule disjE)
...
lemma act_clock_post_happ_spec_intros: 
  "(planning_sem.is_instant_action t a \<Longrightarrow> c (ActStart a) = 0)
\<Longrightarrow> (planning_sem.is_starting_action t a \<Longrightarrow> c (ActStart a) = 0)
\<Longrightarrow> (planning_sem.is_ending_action t a \<Longrightarrow> act_clock_pre_happ_spec c (ActStart a) t)
\<Longrightarrow> (planning_sem.is_not_happening_action t a \<Longrightarrow> act_clock_pre_happ_spec c (ActStart a) t)
\<Longrightarrow> act_clock_post_happ_spec c (ActStart a) t"

    "(planning_sem.is_instant_action t a \<Longrightarrow> c (ActEnd a) = 0) 
\<Longrightarrow> (planning_sem.is_starting_action t a \<Longrightarrow> act_clock_pre_happ_spec c (ActEnd a) t) 
\<Longrightarrow> (planning_sem.is_ending_action t a \<Longrightarrow> c (ActEnd a) = 0) 
\<Longrightarrow> (planning_sem.is_not_happening_action t a \<Longrightarrow> act_clock_pre_happ_spec c (ActEnd a) t) 
\<Longrightarrow> act_clock_post_happ_spec c (ActEnd a) t"
  by (rule planning_sem.action_happening_cases[of t a]; use planning_sem.action_happening_exec_times in force)+
...
let 
  ...
  effecting = v Effecting = Some (card (planning_sem.ending_actions_at t));
...
let 
  ...
  effecting = v Effecting = Some (card (ending_actions_before n i));
...
let 
  ...
  effecting = v Effecting = Some (card (planning_sem.ending_actions_at t) + 1);
...
let 
  ...
  effecting = v Effecting = Some (card (planning_sem.ending_actions_at t) + card (starting_actions_before n j));
...
let 
  ...
  effecting = v Effecting = Some (card (planning_sem.ending_actions_at t) + card (planning_sem.starting_actions_at t));
...
let 
  ...
  prop_state = (\<forall>p. PropVar p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropVar p) = Some (ending_part_updated_prop_state n j p));

  active = (v ActsActive = Some (int (updated_active_during n j)));
  effecting = v Effecting = Some (card (planning_sem.starting_actions_at t) + card (ending_actions_after n j));

  not_upd_loc = (\<forall>i. j \<le> i \<longrightarrow> i < length actions \<longrightarrow> is_ending_index t i \<longrightarrow> L ! Suc i = (EndInstant (actions ! i)));
  upd_loc = (\<forall>i. i < j \<longrightarrow> is_ending_index t i \<longrightarrow> L ! Suc i = (Off (actions ! i)))
...
let 
  ...
  effecting = v Effecting = Some (card (planning_sem.starting_actions_at t));
...
let 
  ...
  locked = (\<forall>p. PropLock p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropLock p) = Some (int (updated_locked_during n j p)));
  effecting = v Effecting = Some (card (starting_actions_after n j));

  not_upd_loc = (\<forall>i. j \<le> i \<longrightarrow> i < length actions \<longrightarrow> is_starting_index t i \<longrightarrow> L ! Suc i = (StartInstant (actions ! i)));
  upd_loc = (\<forall>i. i < j \<longrightarrow> is_starting_index t i \<longrightarrow> L ! Suc i = (PostStart (actions ! i)))
in start_end_invs n Lvc
  \<and> locked
  \<and> effecting
  \<and> not_upd_loc
...
let 
  ...
  locked = (\<forall>p. PropLock p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropLock p) = Some (int (planning_sem.locked_after t p)));
  effecting = v Effecting = Some 0;

  starting_loc = (\<forall>i < length actions. is_starting_index t i \<longrightarrow> L ! Suc i = (PostStart (actions ! i)))
in start_end_invs n Lvc
  \<and> locked
  \<and> effecting
  \<and> starting_loc"
...
definition "inv_check_cond n j Lvc \<equiv>
let 
  t = time_index \<pi> n;
  (L, v, c) = Lvc;
  ...
  upd_loc = (\<forall>i. i < j \<longrightarrow> is_starting_index t i \<longrightarrow> L ! Suc i = (Running (actions ! i)));
  not_upd_loc = (\<forall>i. j \<le> i \<longrightarrow> i < length actions \<longrightarrow> is_starting_index t i \<longrightarrow> L ! Suc i = (PostStart (actions ! i)))
...
definition "inv_check_post n j \<equiv> inv_check_cond n (Suc j)"
...
lemma init_planning_state_propsE:
  assumes "init_planning_state_props x"
      and "\<And>L v c. x = (L, v, c) \<Longrightarrow> Lv_conds L v \<Longrightarrow> v ActsActive = Some 0 \<Longrightarrow> v Effecting = Some 0 
      \<Longrightarrow> L = Planning # map Off actions \<Longrightarrow> (\<forall>p\<in>set init. v (PropVar p) = Some 1) 
      \<Longrightarrow> (\<forall>x\<in>actual_variables - ({PlanningLock, ActsActive} \<union> PropVar ` set init). v x = Some 0) 
      \<Longrightarrow> (\<forall>x. x \<notin> actual_variables \<longrightarrow> v x = None) \<Longrightarrow> c = (\<lambda>_. 0) \<Longrightarrow> thesis"
    shows thesis 
...
lemma init_planning_state_propsI:
  assumes "x = (L, v, c)" 
    ...
    "\<And>p. p\<in>set init \<Longrightarrow> v (PropVar p) = Some 1" 
    "\<And>x. x \<in> actual_variables \<Longrightarrow> x \<notin> {PlanningLock, ActsActive} \<Longrightarrow> x \<notin> PropVar ` set init \<Longrightarrow> v x = Some 0" 
    "\<And>x. x \<notin> actual_variables \<Longrightarrow> v x = None" "c = (\<lambda>_. 0)"
...
lemma init_planning_state_props'E:
  assumes "init_planning_state_props' x"
      and "\<And>L v c. x = (L, v, c) \<Longrightarrow> Lv_conds L v \<Longrightarrow> v ActsActive = Some 0 \<Longrightarrow> v Effecting = Some 0 
      \<Longrightarrow> L = Planning # map Off actions \<Longrightarrow> (\<forall>p. PropVar p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropVar p) = Some (prop_state (set init) p)) 
      \<Longrightarrow> (\<forall>p. PropLock p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropLock p) = Some 0) \<Longrightarrow> (\<forall>i<length actions. c (ActStart (actions ! i)) = 0) 
      \<Longrightarrow> (\<forall>i<length actions. c (ActEnd (actions ! i)) = 0) \<Longrightarrow> thesis"
    shows thesis 
...
lemma init_planning_state_props'I:
  assumes "x = (L, v, c)" 
    ...
    "v Effecting = Some 0" 
    "L = Planning # map Off actions " 
    "(\<forall>p. PropVar p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropVar p) = Some (prop_state (set init) p))" 
    "(\<forall>p. PropLock p \<in> dom (map_of nta_vars) \<longrightarrow> v (PropLock p) = Some 0)" 
    "(\<forall>i<length actions. c (ActStart (actions ! i)) = 0)" 
    "(\<forall>i<length actions. c (ActEnd (actions ! i)) = 0)"
  shows "init_planning_state_props' x"  
  apply (subst assms(1)) 
...
lemma Lv_condsE:
  assumes "Lv_conds L v"
    and "length L = Suc (length actions) \<Longrightarrow> L ! 0 = Planning 
    \<Longrightarrow> Simple_Network_Language.bounded (map_of nta_vars) v 
    \<Longrightarrow> v PlanningLock = Some 1 \<Longrightarrow> thesis"
  shows thesis
...
lemma happening_pre_pre_delayE: 
  ...
  and "\<And>L v c. x = (L, v, c) 
    ...
    \<Longrightarrow> v Effecting = Some 0
...
lemma happening_postI:
  assumes "x = (L, v, c)"
  and "Lv_conds L v"
    "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (prop_state_after_happ t p)"
    "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (planning_sem.locked_after (time_index \<pi> t) p))"
    "v ActsActive = Some (int (planning_sem.active_after (time_index \<pi> t)))"
    "v Effecting = Some 0"
    "\<And>i. i < length actions \<Longrightarrow> planning_sem.closed_active_count (time_index \<pi> t) (actions ! i) = 0 \<Longrightarrow> L ! Suc i = Off (actions ! i)"
    "\<And>i. i < length actions \<Longrightarrow> planning_sem.closed_active_count (time_index \<pi> t) (actions ! i) = 1 \<Longrightarrow> L ! Suc i = Running (actions ! i)"
    "\<And>i. i < length actions \<Longrightarrow> act_clock_post_happ_spec c (ActStart (actions ! i)) (time_index \<pi> t)"
    "\<And>i. i < length actions \<Longrightarrow> act_clock_post_happ_spec c (ActEnd (actions ! i)) (time_index \<pi> t)"
...
lemma happening_invs_dests:
  ...
  shows "Lv_conds L v"
    "i < length actions \<Longrightarrow> is_ending_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActStart (actions ! i)) (time_index \<pi> n)"
    "i < length actions \<Longrightarrow> is_starting_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActEnd (actions ! i)) (time_index \<pi> n)"
    "i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActStart (actions ! i)) (time_index \<pi> n)"
    "i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActEnd (actions ! i)) (time_index \<pi> n)"
    "i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> planning_sem.closed_active_count (time_index \<pi> n) (actions ! i) = 0 \<Longrightarrow> L ! Suc i = Off (actions ! i)"
    "i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> planning_sem.closed_active_count (time_index \<pi> n) (actions ! i) = 1 \<Longrightarrow> L ! Suc i = Running (actions ! i)"
  using assms 
...
lemma happening_invsI:
  assumes "x = (L, v, c)"
    "Lv_conds L v"
    "\<And>i. i < length actions \<Longrightarrow> is_ending_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActStart (actions ! i)) (time_index \<pi> n)"
    "\<And>i. i < length actions \<Longrightarrow> is_starting_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActEnd (actions ! i)) (time_index \<pi> n)"
    "\<And>i. i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActStart (actions ! i)) (time_index \<pi> n)"
    "\<And>i. i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> act_clock_pre_happ_spec c (ActEnd (actions ! i)) (time_index \<pi> n)"
    "\<And>i. i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> planning_sem.closed_active_count (time_index \<pi> n) (actions ! i) = 0 \<Longrightarrow> L ! Suc i = Off (actions ! i)"
    "\<And>i. i < length actions \<Longrightarrow> is_not_happening_index (time_index \<pi> n) i \<Longrightarrow> planning_sem.closed_active_count (time_index \<pi> n) (actions ! i) = 1 \<Longrightarrow> L ! Suc i = Running (actions ! i)"
...
lemma end_start_preE:
  assumes "end_start_pre n i x"
    ...
    v Effecting = Some (card (ending_actions_before n i)) \<Longrightarrow>
...
lemma end_start_preD:
  ...
  shows "end_start_invs n (L, v, c) \<and>
    v Effecting = Some (card (ending_actions_before n i)) \<and>
...
lemma end_start_pre_dests:
  ...
  shows "end_start_invs n (L, v, c)"
    "v Effecting = Some (card (ending_actions_before n i))"
...
lemma end_start_postE:
  assumes "end_start_post n i x"
    ...
    v Effecting = Some (card (ending_actions_before n (Suc i))) \<Longrightarrow>
...
lemma end_start_post_dests:
  assumes "end_start_post n i x"
    and "x = (L, v, c)"
  shows "end_start_invs n x"
    "v Effecting = Some (card (ending_actions_before n (Suc i)))"
...
lemma happening_post_end_starts_dests:
  assumes "happening_post_end_starts n x"
    and "x = (L, v, c)"
  shows "end_start_invs n (L, v, c)"
    "v Effecting = Some (card (planning_sem.ending_actions_at (time_index \<pi> n)))"
...
lemma instant_starting_cond_dests:
  ...
  "v Effecting = Some (card (planning_sem.ending_actions_at (time_index \<pi> n)) + 1)"
  "(\<forall>i. i \<le> j \<and> is_instant_index (time_index \<pi> n) i \<longrightarrow> c (ActStart (actions ! i)) = 0)"
  "(\<forall>i. i < j \<and> is_instant_index (time_index \<pi> n) i \<longrightarrow> c (ActEnd (actions ! i)) = 0)"
  "(\<forall>i. j < i \<and> i < length actions \<and> is_instant_index (time_index \<pi> n) i \<longrightarrow> act_clock_pre_happ_spec c (ActStart (actions ! i)) (time_index \<pi> n))"
  "(\<forall>i. j \<le> i \<and> i < length actions \<and> is_instant_index (time_index \<pi> n) i \<longrightarrow> act_clock_pre_happ_spec c (ActEnd (actions ! i)) (time_index \<pi> n))"
...
lemma start_start_preI:
  assumes "start_start_invs i (L, v, c)"
    ...
    "v Effecting = Some (card (planning_sem.ending_actions_at (time_index \<pi> i)) + card (starting_actions_before i n))"
    "\<And>k. k < n \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = 0"
    "\<And>k. n \<le> k \<Longrightarrow> k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> act_clock_pre_happ_spec c (ActStart (actions ! k)) (time_index \<pi> i)"
...
lemma start_start_postI:
  assumes "start_start_invs i (L, v, c)"
    ...
    "v Effecting = Some (card (planning_sem.ending_actions_at (time_index \<pi> i)) + card (starting_actions_before i (Suc n)))"
...
lemma happening_post_start_startsI:
  assumes "start_start_invs i (L, v, c)"
    "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (prop_state_after_instant_start_happ i p)"
    "v ActsActive = Some (int (planning_sem.active_during (time_index \<pi> i)))"
    ...
    "\<And>k. k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = 0"
...
lemma happening_post_start_starts_dests:
  ...
  shows "start_start_invs i (L, v, c)"
    "PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (prop_state_after_instant_start_happ i p)"
    "v ActsActive = Some (int (planning_sem.active_during (time_index \<pi> i)))"
    "v Effecting = Some (card (planning_sem.ending_actions_at (time_index \<pi> i)) + card (planning_sem.starting_actions_at (time_index \<pi> i)))"
...
lemma happening_pre_end_ends_dests:
  ...
  shows "end_end_invs i (L, v, c)"
    ...
    "v ActsActive = Some (int (planning_sem.active_during (time_index \<pi> i)))"
    "v Effecting = Some (card (planning_sem.ending_actions_at (time_index \<pi> i)) + card (planning_sem.starting_actions_at (time_index \<pi> i)))"
    "k < length actions \<Longrightarrow> is_ending_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = EndInstant (actions ! k)"
  using assms unfolding happening_pre_end_ends_def Let_def prod.case
  by auto
...
lemma end_end_invs_dests:
  ...
  shows "happening_invs i (L, v, c)"
    ...
    "k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = 0"
    "k < length actions \<Longrightarrow> is_ending_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = 0"
    "k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = 0"
...
lemma end_end_invs_maintained:
  ...
  and syn: "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = v' (PropLock p)"
    "\<And>k. k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = c'(ActStart (actions ! k))"
    "\<And>k. k < length actions \<Longrightarrow> is_ending_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = c' (ActEnd (actions ! k))"
    "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = c' (ActStart (actions ! k))"
    "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = c' (ActEnd (actions ! k))"
    "\<And>k. k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = L' ! Suc k"
    "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = L' ! Suc k"
  shows "end_end_invs i (L', v', c')"
  apply (insert assms(1))
...
lemma end_end_preI:
  assumes "end_end_invs i (L, v, c)"
    "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (ending_part_updated_prop_state i n p)"
     "v ActsActive = Some (int (updated_active_during i n))"
...
lemma end_end_pre_dests:
  ...
  shows "end_end_invs i (L, v, c)"
    "PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (ending_part_updated_prop_state i n p)"
    "v ActsActive = Some (int (updated_active_during i n))"
    "v Effecting = Some (card (planning_sem.starting_actions_at (time_index \<pi> i)) + card (ending_actions_after i n))"
...
lemma end_end_postI:
  assumes "end_end_invs i (L, v, c)"
    "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (ending_part_updated_prop_state i (Suc n) p)"
    "v ActsActive = Some (int (updated_active_during i (Suc n)))"
...
lemma end_end_post_dests:
  ...
  shows "end_end_invs i (L, v, c)"
    "PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (ending_part_updated_prop_state i (Suc n) p)"
    "v ActsActive = Some (int (updated_active_during i (Suc n)))"
    "v Effecting = Some (card (planning_sem.starting_actions_at (time_index \<pi> i)) + card (ending_actions_after i (Suc n)))"
...
lemma happening_post_end_endsI:
  assumes "end_end_invs i (L, v, c)"
    "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (prop_state_after_happ i p)"
...
lemma happening_post_end_ends_dests:
  ...
  shows "end_end_invs i (L, v, c)"
    "PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (prop_state_after_happ i p)"
    "v ActsActive = Some (int (planning_sem.active_during_minus_ended (time_index \<pi> i)))"
    "v Effecting = Some (card (planning_sem.starting_actions_at (time_index \<pi> i)))"
...
lemma start_end_invsI:
  assumes "happening_invs i (L, v, c)"
    "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = Some (prop_state_after_happ i p)"
    "v ActsActive = Some (int (planning_sem.active_after (time_index \<pi> i)))"
    "\<And>k. k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = 0"
    "\<And>k. k < length actions \<Longrightarrow> is_ending_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = 0"
    "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = 0"
    "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = 0"
...
lemma start_end_invs_maintained:
  assumes prev: "start_end_invs i (L, v, c)"
      ...
      and syn: "v ActsActive = v' ActsActive"
        "\<And>p. PropVar p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropVar p) = v' (PropVar p)"
        "\<And>k. k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = c' (ActStart (actions ! k))"
        "\<And>k. k < length actions \<Longrightarrow> is_ending_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = c' (ActEnd (actions ! k))"
        "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActStart (actions ! k)) = c' (ActStart (actions ! k))"
        "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> c (ActEnd (actions ! k)) = c' (ActEnd (actions ! k))"
        "\<And>k. k < length actions \<Longrightarrow> is_ending_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = L' ! Suc k"
        "\<And>k. k < length actions \<Longrightarrow> is_instant_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = L' ! Suc k"
      shows "start_end_invs i (L', v', c')"
  apply (insert prev)
...
lemma start_end_preI:
  assumes "start_end_invs i (L, v, c)"
    "v Effecting = Some (card (starting_actions_after i n))"
    "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (updated_locked_during i n p))"
    "\<And>k. n \<le> k \<Longrightarrow> k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = StartInstant (actions ! k)"
    "\<And>k. k < n \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
...
lemma start_end_pre_dests:
  ...
  shows "start_end_invs i (L, v, c)"
    ...
    "k < n \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
  using assms unfolding start_end_pre_def start_end_cond_def by auto

lemma start_end_postI:
  assumes "start_end_invs i (L, v, c)"
    "v Effecting = Some (card (starting_actions_after i (Suc n)))"
    "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (updated_locked_during i (Suc n) p))"
    "\<And>k. (Suc n) \<le> k \<Longrightarrow> k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = StartInstant (actions ! k)"
    "\<And>k. k < (Suc n) \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
...
lemma start_end_post_dests:
  ...
  shows "start_end_invs i (L, v, c)"
    "v Effecting = Some (card (starting_actions_after i (Suc n)))"
    "PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (updated_locked_during i (Suc n) p))"
    "(Suc n) \<le> k \<Longrightarrow> k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = StartInstant (actions ! k)"
    "k < (Suc n) \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
  using assms unfolding start_end_post_def start_end_cond_def by auto

lemma happening_post_start_endsI:
  assumes "start_end_invs i (L, v, c)"
      "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (planning_sem.locked_after (time_index \<pi> i) p))"
      "v Effecting = Some 0"
      "\<And>k. k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
...
lemma happening_post_start_ends_dests:
  ...
  shows "start_end_invs i (L, v, c)"
      ...
      "v Effecting = Some 0"
      "k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> i) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
  using assms unfolding happening_post_start_ends_def by auto
...
lemma inv_check_condI:
  assumes "start_end_invs n (L, v, c)"
    "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (planning_sem.locked_after (time_index \<pi> n) p))"
    "v Effecting = Some 0"
  ...
  shows "inv_check_cond n i (L, v, c)"
  using assms unfolding inv_check_cond_def
  by auto

lemma inv_check_cond_dests:
  assumes "inv_check_cond n i (L, v, c)"
  shows "start_end_invs n (L, v, c)"
    "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (planning_sem.locked_after (time_index \<pi> n) p))"
    "v Effecting = Some 0"
    "\<And>k. k < i \<Longrightarrow> is_starting_index (time_index \<pi> n) k \<Longrightarrow> L ! Suc k = Running (actions ! k)"
    "\<And>k. i \<le> k \<Longrightarrow> k < length actions \<Longrightarrow> is_starting_index (time_index \<pi> n) k \<Longrightarrow> L ! Suc k = PostStart (actions ! k)"
...
lemma inv_check_postI:
  ...
  shows "inv_check_post n i (L, v, c)"
  using assms unfolding inv_check_post_def inv_check_cond_def
  by auto

lemma inv_check_post_dests:
  assumes "inv_check_post n i (L, v, c)"
  shows "start_end_invs n (L, v, c)"
    "\<And>p. PropLock p \<in> dom (map_of nta_vars) \<Longrightarrow> v (PropLock p) = Some (int (planning_sem.locked_after (time_index \<pi> n) p))"
    "v Effecting = Some 0"
    "\<And>k. k < Suc i \<Longrightarrow> is_starting_index (time_index \<pi> n) k \<Longrightarrow> L ! Suc k = Running (actions ! k)"
    ...
    have x: "init_planning_state_props (main_auto_init_edge_effect x)"
      ...
      subgoal for L v c
        ...
        subgoal apply (subst map_upds_apply_nontin) apply force 
          unfolding actual_variables_exact by simp
        ...
        subgoal for x 
          ...
          subgoal apply rule
            apply (frule set_mp[THEN in_actual_variablesI, rotated, of x])
            using init_props by auto
          using actual_variables_exact by auto
        by auto
      done
    moreover
...
proof -
  ...
  proof (rule steps_seq.ext_seq_comp_seq_apply_induct_list_prop_composable[
    ...
    define v' where "v' = (v(Effecting \<mapsto> plus_int (the (v Effecting)) 1, map PropLock (over_all (actions ! (end_indices ! j))) [\<mapsto>] map (\<lambda>x. (the \<circ> v) x - 1) (map PropLock (over_all (actions ! (end_indices ! j))))))"
    ...
    proof (rule updated_bounded[OF _ _ v'_def], goal_cases)
      case 1
      have x: "Simple_Network_Language.bounded (map_of nta_vars) v"
        using esp s 
      ...
      show ?case 
        apply (rule single_upd_bounded)
           apply (rule x)
          apply (rule map_of_nta_vars_Effecting)
        unfolding vE
        apply simp
  ...
  have is_upds: "is_upds v ((Effecting, binop plus_int (var Effecting) (exp.const 1)) # map (inc_prop_lock_ab (- 1)) (over_all (actions ! (end_indices ! j))))
     (v(Effecting \<mapsto> plus_int (the (v Effecting)) 1, map PropLock (over_all (actions ! (end_indices ! j))) [\<mapsto>] map (\<lambda>x. plus_int (the (v x)) (- 1)) (map PropLock (over_all (actions ! (end_indices ! j))))))"
    ...
    show ?case
      ...
      subgoal
        ...
        unfolding s
          ...
          subgoal apply (subst card_ending_actions_before_Suc)
            using eij_ending i eij_in_act apply auto[3]
            unfolding vE 
            apply (subst map_upds_apply_nontin, force)+
            by auto
          ...
          subgoal by (auto simp: nth_actions_unique dest: end_start_pre_dests)
          done
    ...
    thus ?case 
      ...
      subgoal for L v c
        ...
        subgoal apply (drule end_start_post_dests(2), simp)
          apply (subst ending_actions_before_inv[symmetric])
             prefer 4 apply (assumption)
          using i apply simp
          using eip.ys_Suc apply force
          using end_indices_inc_all by simp
        ...
        subgoal by (drule end_start_post_dests) (auto dest: eip.ys_mono[rotated])
        subgoal apply (drule end_start_post_dests(6), simp)
          apply (intro impI allI, elim conjE)
          subgoal for j
            by (cases "j \<le> end_indices ! i") (auto simp: end_indices_inc_all)
          done
        subgoal apply (drule end_start_post_dests(7), simp)
    ...
    thus ?case 
      ...
      subgoal for L v c
        ...
        subgoal apply (drule happening_pre_end_starts_dests(2), simp)
          apply (subst ending_actions_before_inv[symmetric, of _ 0])
          using i apply auto[2]
          using end_indices_inc_all_below apply blast
          using ending_actions_before_0_is_none i by auto
        ...
        subgoal apply (drule happening_pre_end_starts_dests(5), simp)
  ...
  next
    ...
    thus ?case
      ...
      subgoal for L v c 
        ...
        subgoal by (drule end_start_post_dests(1), simp+)
        subgoal apply (subst ending_actions_before_all_is_ending_actions[symmetric])
          using i apply simp
          apply (subst ending_actions_before_inv[symmetric, of _ "Suc (end_indices ! (length end_indices - 1))"])
             apply (rule i)
          apply (rule Suc_leI)
          using eip.nth_ys_ran apply simp
          using end_indices_inc_all_above apply simp
          by (auto dest: end_start_post_dests)
        subgoal apply (drule end_start_post_dests(3), simp)
        ...
        subgoal apply (intro allI impI)
          subgoal for k
            by (cases "k \<le> end_indices ! (length end_indices - 1)"; 
              force dest!: end_indices_inc_all_above[rotated] end_start_post_dests(4))
          done
      subgoal apply (intro allI impI)
        subgoal for k
          by (cases "k \<le> end_indices ! (length end_indices - 1)"; 
              force dest!: end_indices_inc_all_above[rotated] end_start_post_dests(6))
        done
      done
    done
  next
...
proof -                             
  ...
  next
    ...
    proof (intro guard_append)
      have 1: "\<forall>b\<in>set actions. planning_sem.is_ending_action (time_index \<pi> i) b \<longrightarrow> act_clock_pre_happ_spec c (ActStart b) (time_index \<pi> i)"
      ...
      have 2: "\<forall>b\<in>set actions. planning_sem.is_not_happening_action (time_index \<pi> i) b \<longrightarrow> act_clock_pre_happ_spec c (ActStart b) (time_index \<pi> i)"
        unfolding index_case_conv_action[symmetric] 
        by (blast intro:  instant_pre_dests instant_action_invs_dests happening_invs_dests that)
      have 3: "\<forall>b\<in>set actions. planning_sem.is_starting_action (time_index \<pi> i) b \<longrightarrow> act_clock_pre_happ_spec c (ActEnd b) (time_index \<pi> i)"
        unfolding index_case_conv_action[symmetric] 
        by (blast intro:  instant_pre_dests instant_action_invs_dests happening_invs_dests that)
      have 4: "\<forall>b\<in>set actions. planning_sem.is_not_happening_action (time_index \<pi> i) b \<longrightarrow> act_clock_pre_happ_spec c (ActEnd b) (time_index \<pi> i)"
        unfolding index_case_conv_action[symmetric] 
        by (blast intro:  instant_pre_dests instant_action_invs_dests happening_invs_dests that)
      have 5: "act_clock_pre_happ_spec c (ActStart (actions ! (instant_indices ! j))) (time_index \<pi> i)"
        unfolding index_case_conv_action[symmetric] 
    ...
    show ?case 
      ...
      subgoal for x 
        ...
        subgoal for L v c
          ...
          subgoal apply (frule instant_ending_cond_dests(1)) 
            ...
            apply (rule map_of_nta_vars_Effecting)
            subgoal apply (subst instant_ending_cond_dests(4))
               apply simp
              using instant_ending_cond_dests(4) card_ending_actions_less i iij_instant_index iij_ran by fastforce
            subgoal apply (subst instant_ending_cond_dests(4))
               apply simp
              using instant_ending_cond_dests(4) card_ending_actions_less i iij_instant_index iij_ran by fastforce
      ...
      subgoal by (subst map_upds_apply_nontin, force)+ (use instant_ending_cond_dests(4) in fastforce)
  ...
  subgoal for x 
    ...
    subgoal for L v c
      ...
      subgoal by (auto intro!: instant_starting_cond_dests(5) elim: nat_leE)
      subgoal using instant_starting_cond_dests(6) nth_actions_unique  by (auto elim: nat_leE)
      subgoal using nth_actions_unique by (auto dest!: instant_starting_cond_dests(7)) 
      subgoal using nth_actions_unique by (auto dest!: instant_starting_cond_dests(8)) 
      ...
      subgoal by (auto dest!: instant_starting_cond_dests(1,10) instant_action_invs_dests(1) happening_invs_dests(1) dest: Lv_conds_dests simp: iij_ran)
      done
    done
  subgoal for x 
    apply (induction x)
    subgoal for L v c
      ...
      subgoal apply (intro guard_append)
        subgoal 
          ...
          by (auto dest!: instant_starting_cond_dests(5))
        subgoal 
        ...
        subgoal apply (rule instant_action_sat_mutex_end)
          ...
          by (auto dest!: instant_starting_cond_dests(8) simp: iij_instant_index iij_ran )
        subgoal apply (rule instant_action_sat_mutex_end)
                 apply (rule iij_in_act)
                apply (rule iij_instant)
          unfolding set_conv_nth
          ...
          subgoal
            ...
            subgoal using planning_sem.active_before_less_if_scheduled iij_instant iij_in_act card_action_set planning_sem.action_happening_case_defs 
              by (fastforce dest:instant_pre_dests(3))
                  apply (rule map_of_nta_vars_Effecting)
            subgoal apply (subst instant_pre_dests, assumption) by simp
            subgoal apply (subst instant_pre_dests, assumption) using card_ending_actions_less[OF i] iij_instant_index iij_ran by fastforce 
      ...
      subgoal 
        ...
        apply (rule is_upds.intros(2))
         ...
         apply (intro exI conjI)
           apply simp
          apply (rule check_bexp_is_val.intros)
           apply (rule check_bexp_is_val.intros(12)[where v= "the (v Effecting)"])
           apply (drule instant_pre_dests(4), simp)
          apply (subst is_val.simps)
          apply simp
         apply simp
        apply (rule is_upds_appendI)
  ...
  next                                                        
    ...
    then show ?case 
      ...
      subgoal for L v c
        ...
        subgoal using instant_post_dests by fast
...
proof -
  ...
  proof (rule steps_seq.ext_seq_comp_seq_apply_induct_list_prop_composable[
            ...
            subgoal apply (erule Lv_conds_maintained)
              ...
              subgoal apply (rule upds_map_bounded[rotated])
                ...
                subgoal using updated_active_before_less_if_starting[OF i sij_starting_index sij_ran] by simp
                 ...
                 apply (fastforce simp: start_start_pre_dests)
                apply (subst start_start_pre_dests, assumption)
                using card_ending_actions_and_starting_before_less sij_starting_index sij_ran i by fastforce
              done
    ...
    next
     ...
     have v_pre_conds_sat: "Simple_Expressions.check_bexp v' (bexp_and_all (map (is_prop_ab 1) (pre (at_start (actions ! (start_indices ! j)))))) True"
        if prop_state: "\<forall>p. PropVar p \<in> dom (map_of nta_vars) \<longrightarrow> v' (PropVar p) = Some (starting_part_updated_prop_state i (start_indices ! j) p)" for v'
      proof -
        { fix p
          assume p: "p \<in> set (pre (at_start (actions ! (start_indices ! j))))"
          have  "PropVar p \<in> dom (map_of nta_vars)" using  p sij_in_act p map_of_nta_vars_action_start_pre by auto
          hence "v' (PropVar p) = Some (starting_part_updated_prop_state i (start_indices ! j) p)" using prop_state * by auto
          moreover
          ...
          ultimately
          have "v' (PropVar p) = Some 1" by simp
      ...
      proof -
        { fix p
          ...
          hence "p \<notin> planning_sem.plan_invs_during (time_index \<pi> i)" using planning_sem.snap_does_not_delete_inv sij_starting unfolding planning_sem.action_happening_case_defs by auto
          hence "planning_sem.locked_during (time_index \<pi> i) p = 0" using planning_sem.in_invs_during_iff_locked_during by blast
          moreover
          have "PropLock p \<in> dom (map_of nta_vars)" using map_of_nta_vars_action_start_del_lock p sij_in_act by auto
          ultimately
          have "v' (PropLock p) = Some 0" using locked by simp
      ...
      have is_upds: "is_upds v 
        ((ActsActive, binop plus_int (var ActsActive) (exp.const 1)) 
       ...
       (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1, 
          ...
          map PropVar ?dels [\<mapsto>] map (\<lambda>x. 0) ?dels,
          map PropVar ?adds [\<mapsto>] map (\<lambda>x. 1) ?adds))"
      proof (rule is_upds.intros)

        have def: "\<exists>x. v ActsActive = Some x" 
                  "\<exists>x. v Effecting = Some x" using 2 start_start_pre_dests(3,4) by fastforce+
        have is_val: "is_val v (var ActsActive) (the (v ActsActive))"
                     "is_val (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1)) (var Effecting) (the (v Effecting))"
            using def
            by (auto intro: check_bexp_is_val.intros)

        show "is_upd v (ActsActive, binop plus_int (var ActsActive) (exp.const 1)) (v(ActsActive \<mapsto> the (v ActsActive) + 1))"
          unfolding is_upd_def
          by (auto intro: check_bexp_is_val.intros is_val)
        show "is_upds (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1)) 
            ((Effecting, binop plus_int (var Effecting) (exp.const 1)) 
            ...
            (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1,
                ...
                map PropVar ?adds [\<mapsto>] map (\<lambda>x. 1) ?adds))"
        proof (rule is_upds.intros)
          show "is_upd (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1)) 
              (Effecting, binop plus_int (var Effecting) (exp.const 1)) 
              (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) 1, 
                  Effecting \<mapsto> plus_int (the (v Effecting)) 1))"
            unfolding is_upd_def
    ...
    proof (rule guard_append)
      ...
      have 4: "\<forall>b\<in>set actions. planning_sem.is_not_happening_action (time_index \<pi> i) b \<longrightarrow> act_clock_pre_happ_spec c (ActEnd b) (time_index \<pi> i)"
        unfolding index_case_conv_action[symmetric] 
        by (blast intro: start_start_pre_dests start_start_invs_dests happening_invs_dests ssp)
      have 5: "act_clock_pre_happ_spec c (ActStart (actions ! (start_indices ! j))) (time_index \<pi> i)"
        unfolding index_case_conv_action[symmetric] 
  ...
  next
    ...
    hence "planning_sem.starting_actions_at (time_index \<pi> i) = {}"
      apply -
      unfolding start_indices planning_sem.starting_actions_at_def is_starting_index_def 
      apply (subst set_conv_nth)
      by auto
    hence no_starting: "planning_sem.starting_actions_at (time_index \<pi> i) = {}"
           "planning_sem.starting_snaps_at (time_index \<pi> i) = {}" 
      using planning_sem.starting_snaps_at_def by auto

    have not_starting: "\<not>is_starting_index (time_index \<pi> i) j" if "j < length actions" for j 
...
lemma end_ends_possible:
  assumes "abs_renum.urge_bisim.A.steps xs \<and> happening_pre_end_ends i (last xs)"
      and i: "i < length (htpl \<pi>)"
      and end_indices: "end_indices = (filter (is_ending_index (time_index \<pi> i)) [0..<length actions])"
    shows "abs_renum.urge_bisim.A.steps ((ext_seq \<circ> seq_apply) (map end_edge_effect end_indices) xs) \<and> happening_pre_start_ends i (last ((ext_seq \<circ> seq_apply) (map end_edge_effect end_indices) xs))"
proof -
  interpret eip: filter_sorted_distinct_list "[0..<length actions]" "is_ending_index (time_index \<pi> i)" end_indices
    apply (unfold_locales)
    using end_indices by auto
  ...
  proof (rule steps_seq.ext_seq_comp_seq_apply_induct_list_prop_composable[
    ...
    have *: "(actions ! (end_indices ! j)) \<in> planning_sem.ending_actions_at (time_index \<pi> i)" using j by blast
    
    hence eij_ending: "planning_sem.is_ending_action (time_index \<pi> i) (actions ! (end_indices ! j))"  
      and eij_in_act[intro]: "actions ! (end_indices ! j) \<in> set actions"  using * planning_sem.ending_actions_at_def j by auto

    have eij_ending_index: "is_ending_index (time_index \<pi> i) (end_indices ! j)" apply (insert eij_set) apply (subst (asm) (2) end_indices) by simp
        ...
        subgoal apply (frule end_end_pre_dests(1))
          ...
          subgoal apply (rule upds_map_bounded')
            ...
            subgoal apply (subst end_end_pre_dests, assumption)
              ...
              by auto
               apply simp
            using map_of_nta_vars_action_end_del map_of_nta_vars_action_end_add eij_in_act by auto
                     apply auto[4]
          subgoal apply (intro strip)?
            apply (subst nth_list_update_neq)
    ...
    next
      ...
      have v_pre_conds_sat: "Simple_Expressions.check_bexp v' (bexp_and_all (map (is_prop_ab 1) (pre (at_end (actions ! (end_indices ! j)))))) True"
        if prop_state: "\<forall>p. PropVar p \<in> dom (map_of nta_vars) \<longrightarrow> v' (PropVar p) = Some (ending_part_updated_prop_state i (end_indices ! j) p)" for v'
      proof -
        { fix p
          assume p: "p \<in> set (pre (at_end (actions ! (end_indices ! j))))"
          have  "PropVar p \<in> dom (map_of nta_vars)" using  p eij_in_act p map_of_nta_vars_action_end_pre by auto
          hence "v' (PropVar p) = Some (ending_part_updated_prop_state i (end_indices ! j) p)" using prop_state * by auto
          moreover
      ...
      proof -
        { fix p
          ...
          hence "p \<notin> planning_sem.plan_invs_during (time_index \<pi> i)" using planning_sem.snap_does_not_delete_inv eij_ending unfolding planning_sem.action_happening_case_defs by auto
          hence "planning_sem.locked_during (time_index \<pi> i) p = 0" using planning_sem.in_invs_during_iff_locked_during by blast
          moreover
          have "PropLock p \<in> dom (map_of nta_vars)" using map_of_nta_vars_action_end_del_lock p eij_in_act by auto
          ultimately
          have "v' (PropLock p) = Some 0" using locked by simp
      ...
      have is_upds: "is_upds v ((ActsActive, binop plus_int (var ActsActive) (exp.const (- 1))) # (Effecting, binop plus_int (var Effecting) (exp.const (- 1))) # map (set_prop_ab 0) (dels (at_end (actions ! (end_indices ! j)))) @ map (set_prop_ab 1) (adds (at_end (actions ! (end_indices ! j)))))
     (v(ActsActive \<mapsto> plus_int (the (v ActsActive)) (- 1), Effecting \<mapsto> plus_int (the (v Effecting)) (- 1), map PropVar (dels (at_end (actions ! (end_indices ! j)))) [\<mapsto>] map (\<lambda>x. 0) (map PropVar (dels (at_end (actions ! (end_indices ! j))))),
          map PropVar (adds (at_end (actions ! (end_indices ! j)))) [\<mapsto>] map (\<lambda>x. 1) (map PropVar (adds (at_end (actions ! (end_indices ! j)))))))"
      proof (rule is_upds.intros(2)[of _ _ "(v(ActsActive \<mapsto> plus_int (the (v ActsActive)) (- 1)))"], goal_cases)
        case 1
        have vA: "v ActsActive = Some (the (v ActsActive))"
      ...
      next
        case 2
        then show ?case 
        proof (rule is_upds.intros(2)[of _ _ "v(ActsActive \<mapsto> plus_int (the (v ActsActive)) (- 1), Effecting \<mapsto> plus_int (the (v Effecting)) (- 1))"], goal_cases)
          case 1
      ...
      show ?case 
        ...
        unfolding TAG_def
                    apply (subst conv_trans[where p = "Suc (end_indices ! j)"])
                     apply (simp add: eij_ran length_actual_autos)
  ...
  next
    ...
    hence "planning_sem.ending_actions_at (time_index \<pi> i) = {}"
      apply -
      unfolding end_indices planning_sem.ending_actions_at_def is_ending_index_def 
      apply (subst set_conv_nth)
      by auto
    hence no_ending: "planning_sem.ending_actions_at (time_index \<pi> i) = {}"
           "planning_sem.ending_snaps_at (time_index \<pi> i) = {}" 
      using planning_sem.ending_snaps_at_def by auto

    have not_ending: "\<not>is_ending_index (time_index \<pi> i) j" if "j < length actions" for j 
...
lemma inv_check_possible:
  assumes "abs_renum.urge_bisim.A.steps xs \<and> happening_post_start_ends i (last xs)"
      and i: "i < length (htpl \<pi>)"
      and start_indices: "start_indices = (filter (is_starting_index (time_index \<pi> i)) [0..<length actions])"
    shows "abs_renum.urge_bisim.A.steps ((ext_seq \<circ> seq_apply) (map leave_start_edge_effect start_indices) xs) \<and> happening_post i (last ((ext_seq \<circ> seq_apply) (map leave_start_edge_effect start_indices) xs))"
proof -
  interpret sip: filter_sorted_distinct_list "[0..<length actions]" "is_starting_index (time_index \<pi> i)" start_indices
    apply (unfold_locales)
    using start_indices by auto
  ...
  show ?thesis
  proof (rule steps_seq.ext_seq_comp_seq_apply_induct_list_prop_composable[
        where R = "happening_post_start_ends i" 
          and S = "happening_post i"
          and P = "inv_check_cond i o ((!) start_indices)"
          and Q = "inv_check_post i o ((!) start_indices)"
          and fs = "map leave_start_edge_effect start_indices",
    ...
    proof (insert j, induction s; rule context_conjI, goal_cases)
      ...
      have L_len: "length L = Suc (length actions)" 
        using 1
        unfolding comp_def
        apply (subst Lv_conds_dests)
        apply (force  intro: inv_check_cond_dests start_end_invs_dests happening_invs_dests)
        by blast
      hence sij_L: "Suc (start_indices ! j) < length L"
        using sij_ran by simp

      show ?case
        using 1
        unfolding leave_start_edge_effect_alt comp_def
        apply -
      proof(rule inv_check_postI, goal_cases)
        case 1
        then show ?case 
          apply -
          apply (drule inv_check_cond_dests(1))
          apply (erule start_end_invs_maintained)
                  apply (erule happening_invs_maintained)
                       apply (erule Lv_conds_maintained)
          apply auto[8]
          subgoal  apply (intro strip)?
      ...
      next
        case (2 p)
        thus ?case using inv_check_cond_dests by blast
      next
        case 3
        then show ?case using inv_check_cond_dests by blast
      next
        case (4 k)
        then show ?case 
          apply (cases "k = start_indices ! j")
          using sij_L apply simp
          using inv_check_cond_dests by auto
      next
        case (5 k)
        then show ?case 
          apply (cases "k = start_indices ! j")
    ...
    next
      ...
      have v_bounded: "bounded (map_of nta_vars) v" using 2 
        unfolding comp_def
        by (force intro: Lv_conds_dests inv_check_cond_dests start_end_invs_dests happening_invs_dests)

      have v_pre_sat: "check_bexp v (bexp.and (bexp.eq (var Effecting) (exp.const 0)) (bexp_and_all (map (is_prop_ab 1) (over_all (actions ! (start_indices ! j)))))) True"
      proof (subst check_bexp_simps, intro conjI exI, subst conj_absorb, rule HOL.refl)
        show "check_bexp v (bexp.eq (var Effecting) (exp.const 0)) True"
        proof -
          have "v Effecting = Some 0" using 2 unfolding comp_def using inv_check_cond_dests by blast
          thus ?thesis by (simp add: check_bexp_simps is_val_simps)
        qed
        show "check_bexp v (bexp_and_all (map (is_prop_ab 1) (over_all (actions ! (start_indices ! j))))) True"
        proof (intro check_bexp_all ballI)
          { fix p
            assume a: "p \<in> set (over_all (actions ! (start_indices ! j)))"
            have "v (PropVar p) = Some (prop_state_after_happ i p)"
              apply (rule start_end_invs_dests)
               apply (rule inv_check_cond_dests)
              using 2 unfolding comp_def apply simp
              using map_of_nta_vars_action_inv_props sij_in_act a by auto
            moreover
            have "p \<in> planning_sem.upd_state i" using planning_sem.inv_sat_by_upd_state sij_starting sij_in_act i comp_def a by auto
            ultimately
            have "v (PropVar p) = Some 1" using prop_state_after_happ_def i prop_state_def by metis
            hence "is_val v (var (PropVar p)) 1" by (simp add: is_val_simps)
            hence "check_bexp v (bexp.eq (var (PropVar p)) (exp.const 1)) True"
              by (simp add: check_bexp_simps is_val_simps)
          } 
          moreover
          fix b
      ...
      show ?case 
        apply (insert 2)
        unfolding comp_def leave_start_edge_effect_alt
        apply (rule single_step_intro)
        unfolding prod.case
        apply (rule non_t_step_intro[where a = "Internal (STR '''')"])
        unfolding abs_renum.sem_def
        ...
        unfolding TAG_def
                    apply (subst conv_trans[where p = "Suc (start_indices ! j)"])
                     apply (simp add: sij_ran length_actual_autos)
                    apply (rule image_eqI[where x = "leave_start_edge_spec (actions ! (start_indices ! j))"])
                     apply (simp add: leave_start_edge_spec_def)
                    apply (simp add: sij_ran nth_auto_trans)
                   apply (use conv_committed no_committed in simp)
        subgoal using v_pre_sat by blast
        subgoal by simp
        subgoal using conv_invs no_invs by auto
        subgoal using inv_check_cond_dests sij_starting_index sij_ran by simp
        using sij_L  v_bounded by (auto intro: is_upds.intros)
    qed
  next
    case (2 j s)
    then show ?case 
      ...
      apply (induction s)
      subgoal for L v c
        apply (rule inv_check_condI)
        subgoal by (rule inv_check_post_dests)
        subgoal by (rule inv_check_post_dests)
        subgoal by (rule inv_check_post_dests)
        subgoal 
          apply (erule inv_check_post_dests)
           apply (rule ccontr)
          using start_indices_inc_all
          by auto
        subgoal 
  ...
  next
    ...
    then show ?case
      ...
      apply (induction x)
      subgoal for L v c
        apply (intro inv_check_condI)
        subgoal by (rule happening_post_start_ends_dests)
        subgoal by (rule happening_post_start_ends_dests)
        subgoal by (rule happening_post_start_ends_dests)
        subgoal using start_indices_inc_all_below by blast
        subgoal by (auto intro: happening_post_start_ends_dests)
        done
      done
  next
    case (4 x)
    then show ?case
      ...
      apply (induction x)
      subgoal for L v c
        apply (rule happening_postI, simp)
        subgoal by (intro inv_check_post_dests start_end_invs_dests happening_invs_dests)
        subgoal by (intro inv_check_post_dests start_end_invs_dests happening_invs_dests)
        subgoal by (intro inv_check_post_dests start_end_invs_dests happening_invs_dests)
        subgoal by (intro inv_check_post_dests start_end_invs_dests happening_invs_dests)
        subgoal by (intro inv_check_post_dests start_end_invs_dests happening_invs_dests)
        subgoal for k apply (rule planning_sem.closed_active_count_0_happening_casesE[rotated], assumption)
          ...
          apply (blast intro: happening_invs_dests start_end_invs_dests inv_check_post_dests)+
          by simp
        subgoal for k apply (rule planning_sem.closed_active_count_1_happening_casesE[rotated], assumption)
          unfolding index_case_defs[symmetric]
          subgoal apply (erule inv_check_post_dests)
            apply (rule ccontr)
            using start_indices_inc_all_above by auto
          apply (blast intro: happening_invs_dests start_end_invs_dests inv_check_post_dests)
          by simp
        subgoal for k apply (rule act_clock_post_happ_spec_intros)
          unfolding index_case_defs[symmetric]
          by (blast intro: happening_invs_dests start_end_invs_dests inv_check_post_dests)+
        subgoal for k apply (rule act_clock_post_happ_spec_intros)
          unfolding index_case_defs[symmetric]
          by (blast intro: happening_invs_dests start_end_invs_dests inv_check_post_dests)+
        done
...
lemma start_ends_possible:
  assumes "abs_renum.urge_bisim.A.steps xs \<and> happening_pre_start_ends i (last xs)"
      and i: "i < length (htpl \<pi>)"
      and start_indices: "start_indices = (filter (is_starting_index (time_index \<pi> i)) [0..<length actions])"
    shows "abs_renum.urge_bisim.A.steps ((ext_seq \<circ> seq_apply) (map edge_2_effect start_indices) xs) \<and> happening_post_start_ends i (last ((ext_seq \<circ> seq_apply) (map edge_2_effect start_indices) xs))"
proof -
  interpret sip: filter_sorted_distinct_list "[0..<length actions]" "is_starting_index (time_index \<pi> i)" start_indices
    apply (unfold_locales)
    using start_indices by auto
  ...
  proof (rule steps_seq.ext_seq_comp_seq_apply_induct_list_prop_composable[
    ...
    proof (insert j, induction s; rule context_conjI, goal_cases)
      ...
      define v' where "v' = (v(Effecting \<mapsto> plus_int (the (v Effecting)) (- 1), map PropLock (over_all (actions ! (start_indices ! j))) [\<mapsto>] map (\<lambda>x. plus_int (the (v x)) 1) (map PropLock (over_all (actions ! (start_indices ! j))))))"
        
      have variables_locked_after: "v' (PropLock p) = Some (int (updated_locked_during i (Suc (start_indices ! j)) p))" 
        if p_in_vars: "PropLock p \<in> dom (map_of nta_vars)" 
        for p
      ...
      proof (rule updated_bounded[OF _ _ v'_def], goal_cases)
        case 1
        have v_bounded: "bounded (map_of nta_vars) v" using ** by (auto dest: start_end_pre_dests start_end_invs_dests happening_invs_dests Lv_conds_dests)
      ...
      next
        ...
        subgoal for x
          ...
          subgoal for p
            apply (erule ssubst[of x])
            apply (intro exI conjI)
              apply (rule map_of_nta_vars_action_inv[OF sij_in_act])
            using variables_locked_after map_of_nta_vars_action_inv[OF sij_in_act] updated_locked_during_ran apply simp
            using variables_locked_after map_of_nta_vars_action_inv[OF sij_in_act] apply fastforce
            apply (subst variables_locked_after)
            using map_of_nta_vars_action_inv[OF sij_in_act]
    ...
    next
      ...
      have upds: "is_upds (v(Effecting \<mapsto> the (v Effecting) - 1)) (map (inc_prop_lock_ab 1) (over_all (actions ! (start_indices ! j)))) (v(Effecting \<mapsto> plus_int (the (v Effecting)) (- 1), map PropLock (over_all (actions ! (start_indices ! j))) [\<mapsto>] map (\<lambda>x. plus_int (the (v x)) 1) (map PropLock (over_all (actions ! (start_indices ! j))))))"
      proof (rule is_upds_inc_vars)
        show "set (map PropLock (over_all (actions ! (start_indices ! j)))) \<subseteq> dom (v(Effecting \<mapsto> the (v Effecting) - 1))"
        proof (intro subsetI)
          fix x
          assume a: "x \<in> set (map PropLock (over_all (actions ! (start_indices ! j))))"
          from a obtain p where
            x: "x = PropLock p" by auto
          
          have *: "x \<in> dom (map_of nta_vars)" using map_of_nta_vars_action_inv a by blast
        ...
        qed
        show "distinct (map PropLock (over_all (actions ! (start_indices ! j))))" 
          apply (rule distinct_inj_map) using distinct_over_all sij_in_act inj_def by auto
        show "map (inc_prop_lock_ab 1) (over_all (actions ! (start_indices ! j))) = map (\<lambda>v. (v, binop plus_int (var v) (exp.const 1))) (map PropLock (over_all (actions ! (start_indices ! j))))" unfolding inc_prop_lock_ab_def by auto
        show "v(Effecting \<mapsto> plus_int (the (v Effecting)) (- 1), map PropLock (over_all (actions ! (start_indices ! j))) [\<mapsto>] map (\<lambda>x. plus_int (the (v x)) 1) (map PropLock (over_all (actions ! (start_indices ! j))))) =
          v(Effecting \<mapsto> the (v Effecting) - 1, map PropLock (over_all (actions ! (start_indices ! j))) [\<mapsto>] map (\<lambda>x. plus_int x 1) (map ((the \<circ>\<circ>\<circ> fun_upd v) Effecting (Some (the (v Effecting) - 1))) (map PropLock (over_all (actions ! (start_indices ! j))))))"
...
proof -
  let ?seq = "(ext_seq \<circ> seq_apply) (map leave_start_edge_effect (filter (is_starting_index (time_index \<pi> i)) [0..<length actions]))
           ((ext_seq \<circ> seq_apply) (map edge_2_effect (filter (is_starting_index (time_index \<pi> i)) [0..<length actions]))
  ...
  proof (intro allI impI)
    ...
    next
      ...
      consider "L ! p = Off (actions ! n)" | "L ! p = Running (actions ! n)"
        ...
        by blast+
      note c = this
      have "urgent (fst (snd abs_renum.sem) ! p) = {StartInstant (actions ! n), EndInstant (actions ! n), PostStart (actions ! n)} "
        apply (subst sem_alt_def)
        unfolding fst_conv snd_conv
        apply (subst nth_map, simp add: pl)
        unfolding Suc
  ...
  proof - 
    have 1: "0 < sum_list (map length (map (\<lambda>n. [start_edge_effect n, instant_trans_edge_effect n, end_edge_effect n]) (filter (is_instant_index (time_index \<pi> i)) [0..<length actions]))) +
...
let ?seq = "(ext_seq \<circ> seq_apply) (map leave_start_edge_effect (filter (is_starting_index (time_index \<pi> i)) [0..<length actions]))
  ...
  show "abs_renum.urge_bisim.A.steps ?seq \<and> happening_post i (last ?seq)"
    ...
    by (auto intro!: i abs_renum.urge_bisim.A.steps.intros pres')
qed
...
lemma goal_run_is_run: "abs_renum.urge_bisim.A.run (goal_run (last plan_steps))"
proof -
  have x: "goal_state_conds (shd (goal_run (last plan_steps)))"  using all_steps_possible by simp
  
  show ?thesis
  proof (rule abs_renum.urge_bisim.A.run.coinduct[where X = "\<lambda>x. goal_state_conds (shd x) \<and> x = goal_run (shd x)"], goal_cases)
    case 1
  ...
  next
    ...
    have trans: "abs_renum.sem \<turnstile> \<langle>L, v, c\<rangle> \<rightarrow> \<langle>L, v, c\<rangle>"
      apply (rule non_t_step_intro[where a = "Internal (STR '''')"])
      unfolding abs_renum.sem_def
...
lemma valid_plan_imp_form_holds': "abs_renum.sem, abs_renum.a\<^sub>0 \<Turnstile> form"
proof -
  ...
  have plan_steps_alt: "abs_renum.a\<^sub>0 # (tl plan_steps) = plan_steps"
  proof -
     have "abs_renum.a\<^sub>0 = hd plan_steps" unfolding plan_steps_def 
  ...
  have run_alt: "abs_renum.a\<^sub>0 ## (stl (plan_steps @- (goal_run (last plan_steps)))) = (plan_steps @- (goal_run (last plan_steps)))"
  ...
  have steps: "abs_renum.urge_bisim.A.steps plan_steps" using all_steps_possible by blast

  have run: "abs_renum.urge_bisim.A.run (plan_steps @- (goal_run (last plan_steps)))"
  proof (rule Graph_Defs.extend_run')
    show "abs_renum.urge_bisim.A.steps plan_steps" using all_steps_possible by blast
    show "abs_renum.urge_bisim.A.run (goal_run (last plan_steps))" using goal_run_is_run by blast
  ...
  qed
  hence run': "abs_renum.urge_bisim.A.run (abs_renum.a\<^sub>0 ## (stl (plan_steps @- (goal_run (last plan_steps)))))" using run_alt by auto

  have form_holds: "holds (\<lambda>(L, v, _). check_sexp (sexp.loc 0 GoalLocation) L (the \<circ> v)) (goal_run (last plan_steps))"
  proof -
    obtain L v c where
    ...
    hence "L ! 0 = GoalLocation" using goal_state_condsE by force
    hence "check_sexp (sexp.loc 0 GoalLocation) L (the \<circ> v)" by auto
  ...
  show "?thesis" unfolding form_alt 
    ...
    unfolding formula.case
    apply (subst abs_renum.urge_bisim.A.Ex_ev_def)
    apply (subst Sequence_LTL.ev_alt_def)
    using run' run_alt form_holds by blast
qed
end
theorem valid_plan_imp_form_holds:
  assumes "\<exists> \<pi>. valid_plan \<pi> (set init) (set goal) at_start at_end (set \<circ> over_all) lower_sem upper_sem (set \<circ> pre) (set \<circ> adds) (set \<circ> dels) (Rat.of_int \<epsilon>) \<and> plan_actions_in_problem \<pi> (set actions) \<and> no_self_overlap \<pi>" 
  shows "abs_renum.sem,abs_renum.a\<^sub>0 \<Turnstile> form" using valid_plan_imp_form_holds' assms by auto

corollary form_not_sat_imp_no_valid_plan:
  assumes "\<not>abs_renum.sem,abs_renum.a\<^sub>0 \<Turnstile> form"
  shows "\<not>(\<exists> \<pi>. valid_plan \<pi> (set init) (set goal) at_start at_end (set \<circ> over_all) lower_sem upper_sem (set \<circ> pre) (set \<circ> adds) (set \<circ> dels) (Rat.of_int \<epsilon>) \<and> plan_actions_in_problem \<pi> (set actions) \<and> no_self_overlap \<pi>)"
