  ...
  proof -
    ...
    proof -
      { fix c
        ...
        moreover
        have "\<exists>a \<in> actions. snap = at_start a \<or> snap = at_end a" using snap_in_B in_happ_seqE'
          pap[simplified plan_actions_in_problem_def] by fast
        hence "pr \<in> props" using pr_in_pre act_img_actions using fluent_domain unfolding fluent_domain_def act_ref_fluents_def by auto
        ultimately
